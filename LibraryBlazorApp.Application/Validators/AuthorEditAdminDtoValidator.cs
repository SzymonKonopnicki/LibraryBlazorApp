...
public class AuthorEditAdminDtoValidator : AbstractValidator<AuthorEditAdminDto>
{
    public AuthorEditAdminDtoValidator()
    {
        RuleFor(b => b.Name)
    ...
    {
        var result = await ValidateAsync(ValidationContext<AuthorEditAdminDto>.CreateWithOptions((AuthorEditAdminDto)model, x => x.IncludeProperties(propertyName)));
        if (result.IsValid)
