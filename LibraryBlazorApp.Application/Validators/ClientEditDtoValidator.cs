...
public class ClientEditDtoValidator : AbstractValidator<ClientEditAdminDto>
{
    public ClientEditDtoValidator()
    {
        RuleFor(c => c.Name)
    ...
    {
        var result = await ValidateAsync(ValidationContext<ClientEditAdminDto>.CreateWithOptions((ClientEditAdminDto)model, x => x.IncludeProperties(propertyName)));
        if (result.IsValid)
